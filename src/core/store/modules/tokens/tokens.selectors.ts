...
const selectUserAllowanceMap = (state: RootState) => state.tokens.user.userTokensAllowancesMap;
...
export function createToken(props: CreateTokenProps): TokenView {
  ...
  return {
    ...
    icon: tokenData?.icon,
    balance: userTokenData ? userTokenData?.balance ?? '0' : '0',
    balanceUsdc: userTokenData ? userTokenData?.balanceUsdc ?? '0' : '0',
